...
class SaliencyFastRCNNOutputs(object):
    ...
    def _log_accuracy(self):
        ...
        num_false_negative = torch.nonzero((fg_pred_classes == bg_class_ind), as_tuple=False).numel()
        num_accurate = torch.nonzero((pred_classes == self.gt_classes), as_tuple=False).numel()
        fg_num_accurate = torch.nonzero((fg_pred_classes == fg_gt_classes), as_tuple=False).numel()
    ...
    def losses(self):
        ...
        return {
            ...
            "loss_smooth_l1": self.smooth_l1_loss(),
...
def fast_rcnn_inference_single_image(
    ...
    bef_boxes = boxes.detach().cpu().numpy()
    new_boxes = []
    for box in bef_boxes:
        area = (box[2] - box[0]) * (box[3] - box[1])
        if area > AREA_MARGIN:
            box = [0, 0, 0, 0]
        else:
            if box[0] - LL_MARGIN < 0:
                box[0] = box[0] - int(box[0])
            else:
                box[0] = box[0] - LL_MARGIN

            if box[1] - LR_MARGIN < 0:
                box[1] = box[1] - int(box[1])
            else:
                box[1] = box[1] - LR_MARGIN

            if box[2] + UL_MARGIN > 384:
                box[2] = box[2] + int(384 - box[2])
            else:
                box[2] = box[2] + UL_MARGIN

            if box[3] + UR_MARGIN > 384:
                box[3] = box[3] + int(384 - box[3])
            else:
                box[3] = box[3] + UR_MARGIN

        new_boxes.append(box)
        
    result = Instances(image_shape)
    # result.pred_boxes = Boxes(boxes)
    result.pred_boxes = Boxes(torch.Tensor(new_boxes).to('cuda', dtype=torch.int64))
